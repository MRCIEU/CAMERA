...
In this example the estimates are broadly consistent across ancestries, although the EAS estimate is somewhat larger than the others (see the `Qjpval` column). Note that the `All` estimate is slightly more precisely estimated than any of the others because it is combining similar estimates.
...
In this example the estimates are more consistent across ancestries than with the raw instruments (compare the `Qjpval` values), although they are slightly less precise.
...
In this example, compared to above, the `agreement` regression slopes are closer to 1 for most pairs of ancestries.
